...
Page({
  ...
  onReady: function () {
    ...
    //调用初始化请求数据
    that.Request(
      'http://c.m.163.com/nc/article/headline/T1348647853363/20-30.html?from=toutiao&passport=&devId=OPdeGFsVSojY0ILFe6009pLR%2FMsg7TLJv5TjaQQ6Hpjxd%2BaWU4dx4OOCg2vE3noj&size=20&version=5.5.3&spever=false&net=wifi&lat=&lon=&ts=1456985878&sign=oDwq9mBweKUtUuiS%2FPvB015PyTDKHSxuyuVq2076XQB48ErR02zJ6%2FKXOnxX046I&encryption=1&canal=appstore',
      function (res) {
        var sentData = res.data[that.data.newMsgId]
        that.setData({
          "navType[0].keep": sentData
        })
      }
  ...
  onPullDownRefresh: function () {
    ...
    //刷新数据
    that.Request(
      'http://c.m.163.com/nc/article/headline/T1348647853363/' + that.data.pageAdd + '-' + that.data.pageAddEnd + '.html?from=toutiao&passport=&devId=OPdeGFsVSojY0ILFe6009pLR%2FMsg7TLJv5TjaQQ6Hpjxd%2BaWU4dx4OOCg2vE3noj&size=20&version=5.5.3&spever=false&net=wifi&lat=&lon=&ts=1456985878&sign=oDwq9mBweKUtUuiS%2FPvB015PyTDKHSxuyuVq2076XQB48ErR02zJ6%2FKXOnxX046I&encryption=1&canal=appstore',
      function (res) {
        // console.log(res);
        var sentData = res.data['T1348647853363']
        that.setData({
        "navType[0].keep": sentData
        })
        that.data.pageAdd += 10;
        that.data.pageAddEnd += 10;
        console.log(that.data.pageAdd, that.data.pageAddEnd)
      }
    );
    //模拟加载
    setTimeout(function () {
